...
public class Test  extends TestCase {
    ...
    public void testPlanesSort(){
        ...
        for (int i = 0; i < planes.size() - 1; i++) {
            if(planes.get(i).getFuelRate() > planes.get(i + 1).getFuelRate()) {
                isSorted = false;
            }
        }
        assertTrue(isSorted);
    ...
    public void testGetPlanesWithDistance(){
        Company company = new Company();
        ...
        ArrayList<Plane> planes = company.getPlanesWithDistance(2000 , 2300);
