...
public class Company {
    ...
    public void sortPlanesByFuel() {
        Plane temp;
        for (int i = 0; i < planes.size() - 1; i++) {
            for (int j = 0; j < planes.size() - 1; j++) {
                if (planes.get(j).getFuelRate() > planes.get(j + 1).getFuelRate()) {
                    temp = planes.get(j + 1);
    ...
    public ArrayList<Plane> getPlanesWithDistance(int from, int to) {
        ArrayList<Plane> arrayToReturn = new ArrayList<>();
        for (Plane plane : planes) {
            if (plane.getDistance() >= from && plane.getDistance() <= to) {
